...
import { getBusinessTypes, getDistrictStats } from '@/components/portal/data';
...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  return { ...pageMetadata({ title: t('title'), description: t('metaDescription'), path: '/map' }), ...(filtered ? { robots: { index: false, follow: true } } : {}) };
}
...
export default async function MapPage({ searchParams }: Props) {
  ...
  const state: MapState = {
    metric: sp.metric === 'vacancy' ? 'vacancy' : 'price',
  ...
  return (
    <div className="container-page py-8 md:py-12">
      <Breadcrumbs items={[{ name: 'მთავარი', href: '/' }, { name: t('title'), href: '/map' }]} className="mb-4" />
      <header className="mb-6 max-w-2xl">
        <h1 className="text-h2 font-semibold md:text-h1">{t('title')}</h1>
        <p className="mt-2 text-muted">{t('subtitle')}</p>
      </header>
      <MapExplorer initial={state} initialStats={stats} initialGeojson={geojson} businessTypes={types.map((b) => ({ slug: b.slug, nameKa: b.nameKa }))} />
    </div>
  );
}
