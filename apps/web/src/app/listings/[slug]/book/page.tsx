...
export default async function BookPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const tips = await getTranslations('viewings.book.tips');
  return (
    <div className="relative">
      ...
      <div className="container-page relative max-w-6xl py-6 sm:py-10">
        ...
        <div className="mb-6 flex items-start gap-4 sm:mb-8">
          ...
          <div>
            <h1 className="text-[28px] font-bold leading-9 tracking-tight sm:text-[36px] sm:leading-[44px]">{shortTerm ? t('titleShortTerm') : t('title')}</h1>
            <p className="mt-1.5 max-w-2xl text-[15.5px] text-muted">{shortTerm ? t('introShortTerm') : t('intro')}</p>
          </div>
        </div>
