...
export function Dialog({ open, onOpenChange, title, description, children, footer, trigger, className, size = 'md' }: { open?: boolean; onOpenChange?: (o: boolean) => void; title: React.ReactNode; description?: React.ReactNode; children?: React.ReactNode; footer?: React.ReactNode; trigger?: React.ReactNode; className?: string; size?: 'sm' | 'md' | 'lg' | 'xl' }) {
  ...
  return (
    <RDialog.Root open={open} onOpenChange={onOpenChange}>
      ...
      <RDialog.Portal>
        <RDialog.Overlay className="fixed inset-0 z-50 bg-[var(--overlay)] backdrop-blur-sm" />
        <RDialog.Content className={cn('fixed left-1/2 top-1/2 z-50 flex max-h-[90dvh] w-[calc(100%-32px)] -translate-x-1/2 -translate-y-1/2 flex-col rounded-modal border border-border bg-surface text-text shadow-lg focus:outline-none', w, className)}>
          <div className="flex items-start justify-between gap-4 px-6 pb-2 pt-6">
            <div>
              <RDialog.Title className="text-[22px] font-bold leading-tight">{title}</RDialog.Title>
            ...
            <RDialog.Close className="grid size-9 place-items-center rounded-full text-muted transition-colors hover:bg-surface-2 hover:text-text" aria-label="დახურვა">
              <X className="size-4" strokeWidth={1.5} />
            </RDialog.Close>
          </div>
          <div className="overflow-y-auto px-6 py-5">{children}</div>
          {footer && <div className="flex justify-end gap-2 border-t border-border bg-surface-2/60 px-6 py-4">{footer}</div>}
        </RDialog.Content>
      </RDialog.Portal>
    </RDialog.Root>
  );
...
export function Popover({ trigger, children, align = 'start', className, open, onOpenChange }: { trigger: React.ReactNode; children: React.ReactNode; align?: 'start' | 'center' | 'end'; className?: string; open?: boolean; onOpenChange?: (o: boolean) => void }) {
  return (
    <RPopover.Root open={open} onOpenChange={onOpenChange}>
      ...
      <RPopover.Portal>
        <RPopover.Content align={align} sideOffset={6} className={cn('z-50 min-w-56 rounded-2xl border border-border bg-surface p-2 text-text shadow-lg focus:outline-none', className)}>
          {children}
        </RPopover.Content>
      </RPopover.Portal>
    </RPopover.Root>
  ...
  return (
    <RTooltip.Provider delayDuration={250}>
      <RTooltip.Root>
        ...
        <RTooltip.Portal>
          <RTooltip.Content side={side} sideOffset={6} className="z-50 max-w-xs rounded-lg bg-basalt px-3 py-2 text-small text-plaster shadow-md">
            {content}
...
export function Tabs({ tabs, value, defaultValue, onValueChange, className, listClassName }: { tabs: { value: string; label: React.ReactNode; content: React.ReactNode; count?: number }[]; value?: string; defaultValue?: string; onValueChange?: (v: string) => void; className?: string; listClassName?: string }) {
  return (
    <RTabs.Root value={value} defaultValue={defaultValue ?? tabs[0]?.value} onValueChange={onValueChange} className={className}>
      <RTabs.List className={cn('flex w-fit max-w-full gap-1 overflow-x-auto rounded-2xl bg-surface-2 p-1', listClassName)}>
        {tabs.map((t) => (
          <RTabs.Trigger key={t.value} value={t.value} className="flex h-10 items-center gap-2 whitespace-nowrap rounded-xl px-4 text-[15px] font-medium text-muted transition-all hover:text-text data-[state=active]:bg-surface data-[state=active]:text-text data-[state=active]:shadow-sm">
            {t.label}
            {t.count !== undefined && <span className="rounded-full bg-surface-3 px-2 text-[11px] font-semibold tabular text-muted">{t.count}</span>}
          </RTabs.Trigger>
        ))}
      </RTabs.List>
      {tabs.map((t) => (
      ...
      <RToast.Provider swipeDirection="right" duration={4500}>
        ...
        {items.map((t) => (
          <RToast.Root
            ...
            className={cn('flex flex-col gap-1 rounded-2xl border bg-surface px-4 py-3 text-text shadow-lg', t.tone === 'danger' ? 'border-danger' : t.tone === 'success' ? 'border-success' : 'border-border-strong')}
          >
