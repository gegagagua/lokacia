...
import { formatDateKa, formatMoney, type PlansResponse } from '@lokacia/contracts';
    ...
    <Dialog
      ...
      trigger={
        <Button size="sm" variant={active ? 'secondary' : 'ghost'} icon={<Sparkles className="size-3.5 text-accent" strokeWidth={1.5} aria-hidden />}>
          {active ? t('activeUntil', { date: formatDateKa(vipUntil!) }) : t('enable')}
        </Button>
      }
      footer={
        <>
          ...
          <Button onClick={buy} loading={busy} disabled={!selected}>
            {data?.promoActive ? t('enableFree') : selected ? t('pay', { amount: formatMoney(selected.priceMinor) }) : t('enable')}
          </Button>
        </>
      }
    >
      {!data ? (
        ...
        <div className="flex flex-col gap-4">
          <RadioGroup
            ...
            aria-label={t('period')}
            options={options.map((p) => ({ value: p.key, label: `${p.days} ${t('days')} — ${data.promoActive ? '0 ₾' : formatMoney(p.priceMinor)}` }))}
          />
          {data.promoActive && <p className="rounded-button border border-accent bg-accent/10 px-3 py-2 text-small">{t('promo', { date: data.promoUntil ? formatDateKa(data.promoUntil) : '' })}</p>}
          {active && <p className="text-small text-muted">{t('extendHint', { date: formatDateKa(vipUntil!) })}</p>}
