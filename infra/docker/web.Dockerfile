...
# lokacia.ge @lokacia/web (Next.js) production image.
#   docker build -f infra/docker/web.Dockerfile --build-arg NEXT_PUBLIC_APP_URL=https://lokacia.ge -t ghcr.io/<org>/lokacia-web .
# NEXT_PUBLIC_* values are inlined at build time → build one image per environment (staging / prod).
# Follow-up (smaller image, ~150 MB): set `output: 'standalone'` in apps/web/next.config.ts and copy
...
RUN corepack enable
COPY . .
RUN pnpm dlx turbo@2.10.13 prune @lokacia/web --docker
...
RUN pnpm turbo run build --filter=@lokacia/web... && rm -rf apps/web/.next/cache
