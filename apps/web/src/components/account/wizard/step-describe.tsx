...
export function StepDescribe({ form, set, types, errors }: { form: WizardForm; set: (p: Partial<WizardForm>) => void; types: BusinessTypeOption[]; errors: Record<string, string> }) {
  ...
  const generate = async () => {
    ...
    try {
      ...
      const r = await apiFetch<DescribeResponse>('/ai/describe', {
        method: 'POST',
        body: { facts: { title: form.title || suggestTitle(form, types), businessTypes: form.businessTypes, dealType: form.dealType, areaM2: parseNum(form.areaM2) ?? 1, floor: parseNum(form.floor), address: form.address, districtId: form.districtId, lat: form.lat, lng: form.lng, priceMinor: money(form.price) ?? 0, passport } },
      });
  ...
  return (
    <StepSection title={t('heading')}>
      ...
      {!form.title && (
        <Button variant="link" className="-mt-3 w-fit" icon={<Wand2 className="size-4" strokeWidth={1.5} aria-hidden />} onClick={() => set({ title: suggestTitle(form, types) })}>
          {t('titleSuggest')}
        </Button>
      )}
