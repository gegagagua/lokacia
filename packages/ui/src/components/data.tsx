    ...
    <div className={cn('relative overflow-x-auto rounded-card border border-border bg-surface', className)}>
      <table className="w-full border-collapse text-left text-[15px] tabular">
        <thead>
          <tr className="border-b border-border-strong">
            {columns.map((c, i) => {
              ...
              return (
                <th key={c.key} scope="col" aria-sort={active ? (sort!.dir === 'asc' ? 'ascending' : 'descending') : undefined} className={cn('whitespace-nowrap px-3 py-2.5 text-small font-medium text-muted', c.align === 'right' && 'text-right', stickyFirst && i === 0 && 'sticky left-0 bg-surface', c.className)}>
                  {c.sortValue ? (
        ...
        <tbody>
          ...
          {sorted.map((r) => (
            <tr key={rowKey(r)} onClick={onRowClick ? () => onRowClick(r) : undefined} className={cn('border-b border-border last:border-b-0', onRowClick && 'cursor-pointer hover:bg-surface-2')}>
              {columns.map((c, i) => (
                <td key={c.key} className={cn('px-3 py-2.5 align-middle', c.align === 'right' && 'text-right', c.align === 'center' && 'text-center', stickyFirst && i === 0 && 'sticky left-0 bg-surface', c.className)}>
                  {c.cell(r)}
                </td>
              ))}
            </tr>
...
export function Pagination({ page, pages, onPage, className }: { page: number; pages: number; onPage: (p: number) => void; className?: string }) {
  ...
  return (
    <nav aria-label="გვერდები" className={cn('flex items-center gap-1', className)}>
      <button type="button" className="grid size-9 place-items-center rounded-button border border-border hover:bg-surface-2 disabled:opacity-40" disabled={page <= 1} onClick={() => onPage(page - 1)} aria-label="წინა">
        <ChevronLeft className="size-4" strokeWidth={1.5} />
      </button>
      {nums.map((n, i) => (
        <React.Fragment key={n}>
          {i > 0 && n - nums[i - 1]! > 1 && <span className="px-1 text-muted">…</span>}
          <button type="button" aria-current={n === page ? 'page' : undefined} onClick={() => onPage(n)} className={cn('h-9 min-w-9 rounded-button border px-2 tabular', n === page ? 'border-primary bg-primary text-primary-contrast' : 'border-border hover:bg-surface-2')}>
            {n}
          </button>
        </React.Fragment>
      ))}
      <button type="button" className="grid size-9 place-items-center rounded-button border border-border hover:bg-surface-2 disabled:opacity-40" disabled={page >= pages} onClick={() => onPage(page + 1)} aria-label="შემდეგი">
  ...
  return (
    <div className={cn('relative', className)}>
      <input
        ...
        aria-label={label}
        className="h-10 w-full rounded-button border border-border-strong bg-surface px-3 text-text placeholder:text-muted/80 focus:border-focus focus:outline-none"
      ...
      />
      {open && (
        <ul id={`${id}-list`} role="listbox" className="absolute z-40 mt-1 max-h-64 w-full overflow-y-auto rounded-card border border-border bg-surface py-1">
          {value && (
          ...
          {filtered.map((o, i) => (
            <li key={o.value} id={`${id}-${o.value}`} role="option" aria-selected={o.value === value} onMouseDown={() => commit(o.value)} onMouseEnter={() => setActive(i)} className={cn('flex cursor-pointer items-center justify-between gap-2 px-3 py-2 text-[15px]', i === active && 'bg-surface-2', o.value === value && 'font-medium text-primary')}>
              {o.label}
