...
import { PASSPORT_FIELDS, formatArea, formatDateKa, formatMoney, formatNumber, type ListingDetail } from '@lokacia/contracts';
...
export async function ListingSpecs({ listing: l }: { listing: ListingDetail }) {
  ...
  const rows = PASSPORT_FIELDS.flatMap((f) => {
    ...
    return [{ key: f.key, label: f.labelKa, value, unit: typeof v === 'boolean' ? undefined : f.unit, icon: Icon ? <Icon className="size-4" strokeWidth={1.5} aria-hidden /> : undefined, muted: v === false }];
  });
    ...
    <section aria-labelledby="specs-title" className="grid gap-6 md:grid-cols-[minmax(0,5fr)_minmax(0,6fr)]">
      <div className="flex flex-col gap-2">
        ...
        <div className="drawing-grid rounded-card border border-border bg-bg p-4">
          <SpacePlan areaM2={l.areaM2} widthM={p.widthM} depthM={p.depthM} ceilingM={p.ceilingM} powerKw={p.powerKw} outline={p.outline} title={`${t('plan')}: ${formatArea(l.areaM2)}`} />
        </div>
      </div>
      <div className="rounded-card border border-border bg-surface px-4 py-2">
        <SpecRow label={t('area')} value={formatNumber(l.areaM2, Number.isInteger(l.areaM2) ? 0 : 1)} unit="მ²" />
        ...
        {isLease && l.serviceFeeMinor > 0 && <SpecRow label={t('serviceFee')} value={formatMoney(l.serviceFeeMinor, l.currency)} />}
        {isLease && <SpecRow label={t('utilities')} value={l.utilitiesIncluded ? t('yes') : t('no')} muted={!l.utilitiesIncluded} />}
        {l.completionDate && <SpecRow label={t('completion')} value={formatDateKa(l.completionDate)} />}
