...
# lokacia.ge @lokacia/crm (Next.js) production image.
#   docker build -f infra/docker/crm.Dockerfile --build-arg NEXT_PUBLIC_APP_URL=https://lokacia.ge -t ghcr.io/<org>/lokacia-crm .
# NEXT_PUBLIC_* values are inlined at build time → build one image per environment (staging / prod).
# Follow-up (smaller image, ~150 MB): set `output: 'standalone'` in apps/crm/next.config.ts and copy
...
RUN corepack enable
COPY . .
RUN pnpm dlx turbo@2.10.13 prune @lokacia/crm --docker
...
RUN pnpm turbo run build --filter=@lokacia/crm... && rm -rf apps/crm/.next/cache
