...
import { Crm, Ctx, visibleContact, visibleDeal, type CrmCtx } from '../shared/crm-access';
...
export class CrmCallsController {
  ...
  async create(@Ctx() ctx: CrmCtx, @ZBody(callLogSchema) body: z.infer<typeof callLogSchema>) {
    ...
    return this.dbs.org(ctx.orgId, async (tx) => {
      // only records visible to the caller (agents: own contacts/deals) — 404 otherwise
      const contactId = entity === 'contact' ? (await visibleContact(tx, ctx, entityId)).id : (await visibleDeal(tx, ctx, entityId)).contactId;
