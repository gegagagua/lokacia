...
import { DEAL_TYPE_LABELS_KA, formatArea, type DealType } from '@lokacia/contracts';
    ...
    <div className="flex flex-col gap-4 rounded-card border border-border bg-surface p-4 sm:flex-row sm:items-center">
      ...
      <div className="min-w-0 flex-1">
        <div className="mb-1 flex flex-wrap gap-1.5">
          <Badge tone="outline">{DEAL_TYPE_LABELS_KA[listing.dealType as DealType] ?? listing.dealType}</Badge>
          <Badge tone="neutral">{formatArea(listing.areaM2)}</Badge>
        </div>
      ...
      <div className="flex shrink-0 flex-col gap-2 sm:items-end">
        <PriceTag priceMinor={listing.priceMinor} currency={listing.currency} period={listing.pricePeriod as 'month'} areaM2={listing.areaM2} size="sm" />
        {children}
      </div>
    </div>
  );
